function [mapM, mapP, vmapM, vmapP, vmapB, mapB,EToE] = buildMaps(eNodes,nCoord,refTri,DOF)

% Purpose: Connectivity and boundary tables in the K # of Np elements
Nfp = refTri.ndf; Np = refTri.nde; Nfaces=3; K = size(eNodes,1); Nv = size(nCoord,1);
EToV = eNodes(:,1:3);  NODETOL = 1e-5;
%EToE:element-to-element  EToF:element-to-face
[EToE, EToF] = connect2D(EToV);
VX = nCoord(:,1)'; VY = nCoord(:,2)'; 
Fmask = refTri.dof_face';
x = DOF(:,:,1)';y = DOF(:,:,2)';
